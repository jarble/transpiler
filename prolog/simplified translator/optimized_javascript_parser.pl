...
main :- phrase(statements(javascript,"\n",Output)," function matches_pattern(arr,pattern){if(pattern.length !== arr.length){return false;}}"),writeln(Output).


statements_(Lang,A,B) --> top_level_statements(Lang,A,B),ws.
statements_(Lang,A,B) --> ws,"\n",top_level_statements(Lang,A,B),ws.
...
statement(Lang,Indent,if(A,B)) -->
	optional_indent(Lang,Indent),if_without_else(Lang,Indent,expr(Lang,A),statements(Lang,indent(Indent),B)),!.
...
expr(Lang,not_equals(Type,A,B)) --> not_equals_(Lang,Type,dot_expr(Lang,A),dot_expr(Lang,B)).
expr(Lang,equals(Type,A,B)) --> equals_(Lang,Type,dot_expr(Lang,A),dot_expr(Lang,B)).
expr(Lang,or(A,B)) --> or_(Lang,dot_expr(Lang,A),dot_expr(Lang,B)).
expr(Lang,and(A,B)) --> and_(Lang,dot_expr(Lang,A),dot_expr(Lang,B)).
...
dot_expr(Lang,floor(A)) -->
	floor_(Lang,expr(Lang,Arr)),!.
dot_expr(Lang,ceiling(A)) -->
...
dot_expr(Lang,access_array(Arr,In_arr)) -->
    access_array_(Lang,parentheses_expr(Lang,Arr),expr(Lang,In_arr)),!.
...
statement_with_semicolon(Lang,set_var(Type,A,B)) -->
	dot_expr(Lang,A),ws,"=",!,ws,expr(Lang,B).
...
top_level_statements(Lang,Indent,[A|B]) --> top_level_statement(Lang,Indent,A),!,top_level_statement_separator(Lang),statements(Lang,Indent,B).
...
function_call_parameters(Lang,[A]) --> dot_expr(Lang,A).
function_call_parameters(Lang,[A|B]) --> dot_expr(Lang,A),ws,",",!,ws,function_call_parameters(Lang,B).
