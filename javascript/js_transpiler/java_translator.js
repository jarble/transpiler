...
function test_examples(){
	...
	for(lang1 of ["javascript","php","perl","c++","haxe","haskell","erlang","ruby","visual basic .net","python"]){
...
function var_type(input_lang,lang,type){
	...
	if(type[0] == "ArrayList"){
		...
		else if(member(lang,['javascript','coffeescript','python','haxe'])){
			return "float";
		}
...
function generate_code(input_lang,lang,indent,arr){
	...
	*/
		...
		if(arr.length === 4){
			...
			var the_condition = generate_code(input_lang,lang,indent,arr[4]);
			if(member(lang,["python"])){
	...
	,matching_symbols)){
		...
		else if(member(lang,['prolog', 'elixir', 'elm', 'sympy', 'haskell','scheme','smt-lib','minizinc','logtalk','pydatalog','polish notation','reverse polish notation','mathematical notation','emacs lisp','smt-lib','erlang','standard ml','icon','oz','clips','newlisp','hy','sibilant','lispyscript','algol 68','clojure','common lisp','f#','ocaml','ml','racket','nemerle'])){
			to_return = a;
		}
		//console.log(to_return);
	}
		...
		[['haskell'],["function_call","toLower",["$a"]]],
		[['haskell','r'],["function_call","toLower",["$a"]]],
		...
		types[to_return] = "int";
			...
			[["c","c++","r","perl","awk","php","prolog","swift","julia","common lisp"],["function_call","log",["$a"]]],
	...
	// see http://rosettacode.org/wiki/Real_constants_and_functions
		...
		[['c','c++','perl','php','pl/i','octave','prolog','swift','perl'],["function_call","floor",["$a"]]],
		[['go','wolfram'],["function_call","Floor",["$a"]]],
		[['haskell','common lisp'],["function_call","floor",["$a"]]],
		[['ruby'],[".",[["$a"],"floor"]]]
	],matching_symbols)){
		var output = generate_code(input_lang,lang,indent,matching_symbols["$a"]);
		to_return = unparse(input_lang,lang,indent,pattern_array.value,matching_symbols);
		...
		[["python"],[".",["Random",["function_call","fromArray",["$a"]]]]],
	],matching_symbols)){
		var output = generate_code(input_lang,lang,indent,matching_symbols["$a"]);
		to_return = unparse(input_lang,lang,indent,pattern_array.value,matching_symbols);
		types[to_return] = "int";
		...
		[["prolog","php","c","c++","perl","haskell","python"],["function_call","round",["$a"]]],
		[['ruby'],[".",["$a","round"]]]
	],matching_symbols)){
		var output = generate_code(input_lang,lang,indent,matching_symbols["$a"]);
		to_return = unparse(input_lang,lang,indent,pattern_array.value,matching_symbols);
		...
		[['c',"c++",'perl','php','pl/i','octave','swift'],["function_call","ceil",["$a"]]],
		...
		[["c++","haskell","asin","c","perl","prolog","swift","php"],["function_call","asin",["$a"]]],
		...
		[["c++","haskell","c","c++","perl","prolog","swift","php","minizinc"],["function_call","acos",["$a"]]],
		...
		[["c++","c","c++","haskell","perl","prolog","swift","php","minizinc"],["function_call","atan",["$a"]]],
	...
	if(to_return === ""){
		...
		if(!member(input_lang,["php","english","ruby","constraint handling rules","clips","prolog","lua","javascript","jison"]) && types[to_return] == undefined && !is_a_statement(arr[0])){
...
function unparse(input_lang,lang,indent,pattern_array,matching_symbols){
	...
	if(pattern_array[0] === "." && pattern_array.length === 2 && pattern_array[1].length === 2){
		...
		return unparse(input_lang,lang,indent,pattern_array[1][0],matching_symbols)+dot_separator+unparse(input_lang,lang,indent,pattern_array[1][1],matching_symbols);
	}
