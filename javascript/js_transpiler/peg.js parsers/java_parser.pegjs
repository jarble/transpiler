...
statement = a:statement_with_semicolon _ ";" {return ["semicolon",a];} / while / do_while / switch / if_statements / function / class / interface / enum / for / foreach
...
function = mod:access_modifier __ "static" __ a:type __ b:var_name _ "(" _ c:parameters _ ")" _ "{" d:statements "}" {return ["function",mod,a,b[1],c,d];}
...
if_statements =
	...
	"if" _ "(" _ a:expr _ ")" _ "{" _ b:statements _ "}" _ c:elif {return ["if_statements",a,b,c]} /
	"if" _ "(" _ a:expr _ ")" _ "{" _ b:statements _ "}" {return ["if_statement",a,b];}
	// "if" _ "(" _ a:expr _ ")" _ "{" b:statements "}" {return  ["if",a,b];}
	// / "if" _ "(" _ a:expr _ ")" _ b:statement_with_semicolon _ ";" {return  ["if",a,b];}
...
class_statement =
		mod:access_modifier __ "abstract" __ a:type __ b:var_name _ "(" _ c:parameters _ ")" _ ";" {return ["abstract_instance_method",mod,a,b,c]} /
...
statement_with_semicolon =
	a:type __ b:var_name _ "=" _ c:expr {return ["initialize_var",a,b,c]} /
	...
	"System.out.println" _ "(" _ a:expr _ ")" {return ["println",a];} / 
	"System.out.print" _ "(" _ a:expr _ ")" {return ["print",a];} /
	"return" __ a:expr {return ["return",a];}


scalar_type = a:("void"/"int"/"double"/"String"/"char"/"boolean"/"float"/"Object")
...
term
  = head:factor _ symbol:("*" / "/") _ tail:term {
    return	[symbol,head,tail];
    }  / "!" a:dot_expr {return ["not",a];} / dot_expr
...
factor
  = "(" _ a:expr _ ")" { return a; } /
  access_array / function_call / number / "true" / "false" / var_name / string_literal / initializer_list

access_array = a:var_name _ "[" _ b:expr _ "]" {return ["access_array",a,b];}
function_call = a:var_name _ "(" _ b:function_call_parameters _ ")" {return ["function_call",a,b];}

initializer_list = "new" __ type _ a:initializer_list1 { return a; } / initializer_list1

initializer_list1 = "{" _ "}" {return ["initializer_list",[]];} / "{" a:initializer_list_ "}" {return ["initializer_list",a];}
